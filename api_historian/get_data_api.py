...
def get_data_interpolated(lista_tags, start_date, start_hour, end_date, end_hour, interval_min=5):
    with open(PATH_TOKEN, 'r') as token_file:
        api_token = token_file.read().strip()
    headers = {'Accept': 'application/json', 'Authorization': f'Bearer {api_token}'}
    ...
    try:
        for tag in lista_tags:
            ...
            try:
                ...
                print(f"Erro ao consultar API interpolated para a tag {tag}: {e}")
                continue
            except json.JSONDecodeError as e:
                print(f"Erro ao decodificar JSON interpolated para a tag {tag}: {e}")
                continue
